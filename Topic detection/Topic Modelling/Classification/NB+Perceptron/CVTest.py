...
####################TRAIN PICKLE##########################
'''
inputFile = open("dump_2.txt", 'r+')

classifiedMessages = np.loadtxt("dump_2.txt", comments='\\<>=#', delimiter="\t", unpack=False, dtype ='string' )


i=0 
messageArray = pickle.load(open("TransformedMessagesArray.p","rb"))
for message in classifiedMessages[:]:
    print message[-1].replace('#','')
    messageArray.append([str( message[0] ) , str( replaceStringWithConcepts(message[-1].replace('#',' ').replace('\\n',' ').replace('\\','')) )])
    if (i%200==0):
        pickle.dump(messageArray,open("TransformedMessagesArray.p","wb"))
    i+=1
print messageArray
...
labelledMessageArray = []
with open ("mali ebola 10-30 until 11-10.txt", "r") as f:
    for line in f.readlines():
        labelledMessageArray.append( line.split("\t") )
with open ("mali ebola 11-10 until 11-20.txt", "r") as f:
    for line in f.readlines():
        labelledMessageArray.append( line.split("\t") )        
with open ("mali ebola 11-20 until 11-30.txt", "r") as f:
    for line in f.readlines():
        labelledMessageArray.append( line.split("\t") )        


np.random.shuffle(labelledMessageArray)
labelledMessageArray = [ [s[0], s[1].replace('#', '').replace("\n", "")] for s in labelledMessageArray]

for item in labelledMessageArray:
    print item
